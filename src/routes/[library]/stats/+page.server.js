...
export async function load({ params }) {
	...
	const users = {
		...
		nsubscribed: {
			thisMonth: _users.reduce((c, user) => c + (!!user.subscriptions.find(({ type, active }) => active && type.library_slug === params.library) && thisMonth[0] <= user.purchased_on && user.purchased_on < thisMonth[1]), 0),
			thisYear: _users.reduce((c, user) => c + (!!user.subscriptions.find(({ type, active }) => active && type.library_slug === params.library) && thisYear[0] <= user.purchased_on && user.purchased_on < thisYear[1]), 0)
		}
	};
	...
	const items = {
		...
		nValidMagazines: _items.reduce((c, item) => c + (item.magazine?.to > new Date()), 0),
		...
		nAuthors: new Set(_items.reduce(
	...
	return {
		items,
		transactions: { nborrows: (await transaction.findMany({ where: { item: { library_slug: params.library } } })).length },
		users,
	};
}
