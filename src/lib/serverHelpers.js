import { author, item, category, language, publisher } from '$lib/db.js';
...
export async function columns(type) {
  ...
  if (type === 'item') {
    columns = [
      ...
      { id: 'status', type: 'hidden', opts: { value: 'IN' } },
      {
        id: 'publisher',
        type: 'select',
      ...
      { id: 'no_of_pages', type: 'number', hidden: true, important: true },
      {
        id: 'purchase_price',
        type: 'number',
        hidden: true,
        getPlain: (number) => String(number),
        important: true
      },
...
export function transColumns(item = 'both') {
  if (item === 'both') {
    return [
      { id: 'id', readOnly: true },
      { id: 'user' },
      ...
      { id: 'due_at', type: 'date' },
      { id: 'returned_at', type: 'date' },
      { id: 'comments' }
    ].map((data) => ({ ...data, name: data.name || capitalize(data.id) }));
  }
  return [
    { id: 'id', readOnly: true },
    { id: item === 'user' ? 'user' : 'item' },
    { id: 'issued_at', type: 'date' },
    { id: 'due_at', type: 'date' },
    { id: 'returned_at', type: 'date' },
    { id: 'comments' }
